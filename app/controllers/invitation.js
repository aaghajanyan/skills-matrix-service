const { OK, INTERNAL_SERVER_ERROR, BAD_REQUEST, NOT_FOUND, CONFLICT, NO_CONTENT, UNAUTHORIZED, getStatusText } = require('http-status-codes');
...
const { couldNotAddCriteria, internalServerError, addErrorMsg } = require('../helper/errorResponseBodyBuilder');
        ...
        if (!invitation) {
            ...
            if (!user) {
                ...
                try {
                    ...
                    return response.status(INTERNAL_SERVER_ERROR).json(internalServerError(Constants.Controllers.Invitation.COULD_NOT_SEND_EMAIL));
                }
                ...
                return response.status(CONFLICT).json(addErrorMsg(Constants.Controllers.Invitation.EMAIL_ALREADY_EXISTS_USER_MODEL));
            }
        } else {
            return response.status(CONFLICT).json(addErrorMsg(Constants.Controllers.Invitation.EMAIL_ALREADY_EXISTS_INVITATION_MODEL));
        }
    } catch (error) {
        logger.error(error);
        return response.status(INTERNAL_SERVER_ERROR).json(couldNotAddCriteria(Constants.TypeNames.INVITATION.toLowerCase()));
    }
};
